			...
			<form name="frmInfo" method="post" action="category_evn.php" ENCTYPE="multipart/form-data" >
				...
				<table class="admin-table-type1">
					...
					<tbody>
						...
						<?php 
						if($arrParentInfo["list"][0]["cat_depth"] == "1" || $arrInfo["list"][0]['cat_depth'] == "2"){
							$is_show = true;
						}else{
							$is_show = false;
						}
						?>
						<tr <?=$is_show?"":"style='display:none;'"?>>
						  <th>남자 금액</th>
						  <td><div class="inputs"><input type="text" name="cat_man_price" value="<?=$arrInfo["list"][0]['cat_man_price'] != ""?$arrInfo["list"][0]['cat_man_price']:"0"?>" style="width:200px;" class="input" /></div></td>
						</tr>
						<tr <?=$is_show?"":"style='display:none;'"?>>
						  <th>여자 금액</th>
						  <td><div class="inputs"><input type="text" name="cat_woman_price" value="<?=$arrInfo["list"][0]['cat_woman_price'] != ""?$arrInfo["list"][0]['cat_woman_price']:"0"?>" style="width:200px;" class="input" /></div></td>
						</tr>
						<tr <?=$is_show?"":"style='display:none;'"?>>
						  <th>검사항목</th>
						  <td>
							<textarea cols="100" rows="2" name="cat_content"><?=stripslashes($arrInfo["list"][0]['cat_content'])?></textarea>
						  </td>
						</tr>
						<tr>
						  <th>사용여부</th>
