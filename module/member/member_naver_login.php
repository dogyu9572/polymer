...
$data[1]	= "client_id=PGrb_8ayyoHOUXYgBWB7";
$data[2]	= "client_secret=5vv4MsXNsD";
$data[3]	= "redirect_uri=".urlencode($now_url);
$data[4]    = "code=".$code;
$data[5]    = "state=".$state;
...
<?
	...
	if($obj_info->resultcode == "00"){
		if($obj_info->response->id ==""){
			?>
				<script>
			...
			$id = "naver_".$obj_info->response->id; // 카카오 아이디 생성
			//$pw = "naver_".$obj_info->response->id."@pwd"; // 카카오 비밀번호 생성
			...
			if($user_info["total"] > 0){
				...
				$phone = $obj_info->response->mobile;

				//$phone = str_replace("-","",$phone); // 본인인증과 같은 형태 유지 // 현재 - 사용

				$name = $obj_info->response->name;

				$email = $obj_info->response->email;

				$birth = $obj_info->response->birthyear."-".$obj_info->response->birthday;

				if($obj_info->response->gender == "M"){
					$sex = "M";
				}else if($obj_info->response->gender == "F"){
					$sex = "F";
				}
