...
$(document).ready(function() {
	...
	fnViewtype("<?=$arrBoardArticle["list"][0]["category"]?>");
});
	...
	<div class="inbox write_tbl mo_break_write">
		...
		<table>
				...
				<td>
					<div class="inputs">
						<?php
							...
							$i = 0;
							foreach($arrEtc_1 as $key => $val){
								$checked = false;
								if($arrBoardArticle["list"][0]['etc_1'] == $key || ($_REQUEST['mode'] != "modify" && $i == 0)){
									$checked = true;
								}
						?>
						<label class="radio"><input type="radio" name="etc_1" value="<?=$key?>"	<?=$checked?"checked":""?>><i></i><?=$val?></label>
						<?php
								$i++;
							}
						?>
			...
			<tr>
				...
				<td>
					<div class="inputs">
						<label class="radio"><input type="radio" name="category" value="email" <?=$arrBoardArticle["list"][0]['category']!="sms"?"checked":""?> onclick="fnViewtype('email')"><i></i>EMAIL</label>
						<label class="radio"><input type="radio" name="category" value="sms"  <?=$arrBoardArticle["list"][0]['category']=="sms"?"checked":""?> onclick="fnViewtype('sms')"><i></i>SMS</label>
					</div>
				</td>
			</tr>
			<tr>
					...
					if($arrBoardArticle["total_files"]>0 && $_REQUEST['mode']=="modify"){
						for($i=0;$i<$arrBoardArticle["total_files"];$i++){
							if(substr($arrBoardArticle["files"][$i]['re_name'],0,2) != "l_"){
								$reCont .= "<a href=\"".$filepath."uploaded/board/".$arrBoardInfo["list"][0]["boardid"]."/".$arrBoardArticle["files"][$i]['re_name']."\" target=\"_blank\" style=\"display:block; font-size:16px; color:#222; font-weight:500; line-height:44px; background:url(".$filepath."/pub/images/icon_down_b.svg) #F2F7FB no-repeat right 20px center / 13px; padding:0 40px 0 20px; margin-top:20px;\">".$arrBoardArticle["files"][$i]['ori_name']."</a>";
							}
						}
						
						if( strpos($arrBoardArticle["list"][0]['contents'],"@첨부파일@") > 0 ){
							$alertTxt = "저장필수";
						}
						$arrBoardArticle["list"][0]['contents'] = str_replace("@첨부파일@", $reCont, $arrBoardArticle["list"][0]['contents']);
					}
					?>
			...
			<tr class="fileZone">
				<th>첨부파일</th>
				<td>첨부파일은 메일내용에 @첨부파일@ 입력 저장후 한번더 저장 &nbsp; <u style="color:red;"><?=$alertTxt?></u></td>
			</tr>
			<tr class="fileZone">
