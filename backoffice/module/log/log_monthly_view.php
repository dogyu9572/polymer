		...
		<div class="over_tbl mo_break_tbl">
			<div class="bdr_list tac">
				<table>
					...
					?>					
						<tr>
							<td><i class="mo_vw">날짜</i><a href="log_daily_view.php?s_date=<?=$arrList["list"][$i]["yyyy"]?>-<?=$arrList["list"][$i]["mm"]?>-01&e_date=<?=$arrList["list"][$i]["yyyy"]?>-<?=$arrList["list"][$i]["mm"]?>-31"><?=$arrList["list"][$i]["yyyy"]?>-<?=$arrList["list"][$i]["mm"]?></a></td>
							<td><i class="mo_vw">방문수</i><?=number_format($arrList["list"][$i]["sum_hit"])?></td>
							<td><i class="mo_vw">월/년</i><?=$arrList["sum"]!=0?number_format(($arrList["list"][$i]["sum_hit"]/$arrList["sum"])*100,2):"0"?> %</td>
							<td><i class="mo_vw">그래프</i>
							<div style="width:<?=$arrList["sum"]!=0?number_format(($arrList["list"][$i]["sum_hit"]/$arrList["sum"])*100,0):"1"?>%;height:20px;background-color:#CCCCCC"></div>
							</td>
						</tr>
					<?	
						}
