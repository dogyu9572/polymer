...
<div class="container">
	...
	<div class="inbox">
		<div class="bdr_top">
			<div class="left">
				<div class="total">Total : <strong><?=number_format($arrList["sum"])?></strong></div>				
			</div>
		</div>
		...
		<div class="over_tbl mo_break_tbl">
			<div class="bdr_list tac">
				<table>
					...
					?>					
						<tr>
							<td><i class="mo_vw">날짜</i><a href="log_hourly_view.php?s_date=<?=$arrList["list"][$i]["yyyy"]?>-<?=$arrList["list"][$i]["mm"]?>-<?=$arrList["list"][$i]["dd"]?>&e_date=<?=$arrList["list"][$i]["yyyy"]?>-<?=$arrList["list"][$i]["mm"]?>-<?=$arrList["list"][$i]["dd"]?>"><?=$arrList["list"][$i]["yyyy"]?>-<?=$arrList["list"][$i]["mm"]?>-<?=$arrList["list"][$i]["dd"]?> (<?=$arrayWeekName[$arrList["list"][$i]["week"]]?>)</a></td>
							<td><i class="mo_vw">방문수</i><?=number_format($arrList["list"][$i]["hit"])?></td>
							<td><i class="mo_vw">일/월</i><?=$arrList["sum"]!=0?number_format(($arrList["list"][$i]["hit"]/$arrList["sum"])*100,2):"0"?> %</td>
							<td><i class="mo_vw">그래프</i>
							<div style="width:<?=$arrList["sum"]!=0?number_format(($arrList["list"][$i]["hit"]/$arrList["sum"])*100,0):"1"?>%;height:20px;background-color:#CCCCCC"></div>
							</td>
						</tr>
					<?	
						}
