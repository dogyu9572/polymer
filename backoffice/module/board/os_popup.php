...
$arrTotalOsList = getCategoryList_id("os","");
...
function getSelections(){
	...
	if(rows.length>0){
		window.parent.document.getElementById("os").innerHTML = "";
		for(var i=0; i<rows.length; i++){
			window.parent.addSpecificCat("os",rows[i].value,$(rows[i]).attr("data-name"));
		}
		//window.close();
	}else{
		alert('선택된 항목이 없습니다.');
...
<div class="iframe_container pop_iframe_cont">
	...
	<div class="title">OS 추가/삭제</div>

	<div class="inbox">
		<div class="bdr_top">
			<div class="left">
				<div class="total">Total : <strong><?=number_format($arrTotalOsList['total'])?></strong></div>
			</div>
			<div class="right">
				<div class="btns">
				...
				<table>
					...
					<thead>
						<tr>	
							...
							<th class="pc_vw">OS 명</th>
						</tr>
					</thead>
					<tbody>
					<?
					
					if($arrTotalOsList['total'] > 0){
						for($i=0;$i<$arrTotalOsList["total"];$i++){
					?>
						<tr>
							<td class="chkbox"><label class="check notxt"><input type="checkbox" value="<?=$arrTotalOsList["list"][$i]['cat_no']?>" data-name="<?=$arrTotalOsList['list'][$i]['cat_name']?>" name="chk_list" <?=in_array($arrTotalOsList["list"][$i]['cat_no'],$arrOs)?"checked":""?>><i></i></label></td>
							<td><i class="mo_vw">OS 명</i><?=$arrTotalOsList['list'][$i]['cat_name']?></td>
						</tr>
					<?
						}
					}else{
