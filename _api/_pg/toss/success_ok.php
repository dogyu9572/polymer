    ...
    <div>
        <div>
            ...
            <?if($isSuccess){
				...
				## 'cash','card','naver','kakao','vbank','DirectBank'
				$order_state = "6";
				if($PTYPE=="카드"){		
					$payType = "card";		
				}else if($PTYPE=="무통장"){	
				...
				$dblink = SetConn($_conf_db["main_db"]);
					...
					## 주문상태값 변경
					$sql = "update tbl_shop_order_info set order_state='".$order_state."', ipkum_date='".$ipkum_date."', pay_type='".$payType."', tid='".$TID."' where order_no='".$OID."'";
					$rs = mysqli_query($GLOBALS['dblink'], $sql);
					//	echo $sql;

					## 각 상품별 주문상태값 변경
					$sql2 = "update tbl_shop_order_good set order_status='".$order_state."' where order_no='".$OID."'";
			...
			?>
				<script>
					onload = function winClose() {						
						document.location.href="/include/cart3.php?order_no=<?=$OID?>";
					}
				</script>
