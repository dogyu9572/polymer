<?php
    ...
    */
    ...
    $ordr_mony          = $_POST[ "good_mny" ];	//	"1"; // 결제요청금액   ** 1 원은 실제로 업체에서 결제하셔야 될 원 금액을 넣어주셔야 합니다. 결제금액 유효성 검증 **
    /* = -------------------------------------------------------------------------- = */
    ...
    */
    ...
    if ( $res_cd == "0000" )
    {
        ...
        if ( $use_pay_method == "100000000000" )
        {
			...
			##################################################################### 카드결제 ** 계좌이체도 같이 변경해야함 pay_type='DirectBank' 만 다름
			$dblink = SetConn($_conf_db["main_db"]);

			$subQuery = " AND order_no ='".$ordr_idxx."' ";
			$arrOrderInfo = getFreeView("tbl_shop_order_info", $subQuery, $col="*", $scale=0, $offset=0, $orderBy="");
			echo $subQuery;
			echo "total:".$arrOrderInfo['total'];
			
			if($arrOrderInfo['total']>0){

				##################################################################### 수량감소
				$subQuery = " AND order_no ='".$ordr_idxx."' ";
				$arrOrderGoodList = getFreeView("tbl_shop_order_good", $subQuery, $col="*", $scale=0, $offset=0, $orderBy="");

				for($j=0;$j<$arrOrderGoodList["total"];$j++){
					$sql = "update tbl_shop_good set stock=stock-1 where idx='".$arrOrderGoodList['list'][$j]['g_idx']."'";	
					//	echo $sql;
				...
				$sql = "
					update tbl_shop_order_info set order_state='6', tid='".$tno."', pay_type='card', ipkum_date='".date("Y-m-d")."'
					, card_cd = '".$card_cd."', card_name='".$card_name."', card_appno='".$app_no."', card_date=now(), card_quota='".$quota."', card_noinf='".$noinf."'
					where order_no='".$ordr_idxx."'
				";

				$rs = mysqli_query($GLOBALS['dblink'], $sql);
				//	echo $sql;

				if($arrOrderInfo['list'][0]['reserve_state']=="11"){			##1차  예약
					$retUrl = "/member/payment_solo_end.php?orderno=".$ordr_idxx;
					## 2차예약 등록
					setReserveOrderNext($ordr_idxx);

				}else if($arrOrderInfo['list'][0]['reserve_state']=="12"){		##2차  예약
					$retUrl = "/member/payment_solo_end.php?orderno=".$ordr_idxx;
				}else if($arrOrderInfo['list'][0]['reserve_state']=="13"){		##일반 예약
					$retUrl = "/member/payment_solo_end.php?orderno=".$ordr_idxx;
				}else{															##일반 주문
					$retUrl = "/member/payment_end.php?orderno=".$ordr_idxx;
				}			

				$_SESSION["registration"]["order_no"] = $ordr_idxx;		
			}
        ...
        else if ( $use_pay_method == "010000000000" )
        {
			...
			if($arrOrderInfo['total']>0){
				...
				$sql = "update tbl_shop_order_info set order_state='6', tid='".$tno."', pay_type='DirectBank', ipkum_date='".date("Y-m-d")."' where order_no='".$ordr_idxx."'";			
				$rs = mysqli_query($GLOBALS['dblink'], $sql);
				//	echo $sql;
        ...
        if ( $bSucc == "false")
        {
            ...
            */
            ...
            /*
             ...
             --------------------------------------------------------------------------
             */
            $key_data = file_get_contents($_SERVER['DOCUMENT_ROOT'].'\_api\_pg\nhn_kcp\certificate\splPrikeyPKCS8.pem');
