	...
	*/	
	...
	try{
		...
		if($arrResData['PayMethod'] == "VBANK"){ // 가상계좌일 경우
			$bank_code = $arrResData['VbankBankCode']; // 은행코드
			$bank_name = $arrResData["VbankBankName"]; // 은행 명
			$bank_number = $arrResData["VbankNum"]; // 가상계좌 번호
			$bank_exp_date = date("Y-m-d H:i:s", strtotime($arrResData["VbankExpDate"].$arrResData["VbankExpTime"])); // 가상계좌 만료일
		}
	
		//DB연결
		$dblink = SetConn($_conf_db["main_db"]);

		$sql = " UPDATE tbl_board_accept set tid = '$tid', order_state = '1', bank_code = '$bank_code', bank_name = '$bank_name', bank_number = '$bank_number', bank_exp_date = '$bank_exp_date' where order_no = '$moid' ";
		//echo $sql;
		...
		if($rs){
			...
			if($_SESSION[$_SITE["DOMAIN"]]["MEMBER"]["ID"] != ""){ // 로그인되어있을 경우 
				jsGo("/mypage/payment_view.php?orderno=".$moid,"","");
			}else{ // 로그인되어있지 않은경우
				jsGo("/mypage/no_member_payment_view.php?orderno=".$moid,"","");
			}
		}else{
			// db 저장 실패
