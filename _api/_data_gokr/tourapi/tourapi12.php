...
$ch = curl_init();
$url = 'http://apis.data.go.kr/B551011/KorService1/areaBasedList1'; /*URL*/
##	areaCode1		: 지역코드조회
##	categoryCode1	: 서비스분류코드조회
##	areaBasedList1	: 지역기반관광정보조회
$areaCode		= $sidoCode[$_GET['sido']];
$sigunguCode	= $_GET['gugun'];
if(!$areaCode){	$areaCode = "1"; }
if(!$sigunguCode){	$sigunguCode = "1"; }
...
$queryParams .= '&' . urlencode('areaCode') . '=' . urlencode($areaCode);	/* 시도 */
$queryParams .= '&' . urlencode('sigunguCode') . '=' . urlencode($sigunguCode);	/* 구군 */
$queryParams .= '&' . urlencode('contentTypeId') . '=' . urlencode('12');	/* 관광지(12), 문화시설(14), 축제/공연/행사(15) */

//	echo $url . $queryParams."<br/>";
//	http://apis.data.go.kr/B551011/KorService1/areaBasedList1?serviceKey=x02B8Db9kQoN2UzuzZAoVtWA6ZSc88tRo2fvdsSSx68eDsCwDn36llwa4L3x7C%2FWF0qsKvcRDrxGsEi5hAw2eA%3D%3D&pageNo=1&numOfRows=100&MobileOS=ETC&MobileApp=TestApp&areaCode=1&sigunguCode=1&contentTypeId=12
...
//	var_dump( $array['body']['items']['item'][0]);
//	var_dump($array);
/********
